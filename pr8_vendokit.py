# Pandas Analyzer & Visualization – variant 11
import os, sys
import numpy as np
import pandas as pd
...
class VendoKit:
    ...
    def glue_to(self, other_df):
        if self.data.empty: return
        return pd.concat([self.data, other_df], ignore_index=True)

    def splitter(self, by_col="Region"):
        if self.data.empty: return {}
        groups = {}
    ...
    def find_filter(self, keyword="A", sort_by="Sales", min_val=0):
    ...
    def paint(self):
        ...
        plt.savefig("plot_bar_11.png"); plt.close()

        plt.figure()
        self.data.plot(kind="scatter", x="Sales", y="Profit")
        plt.title("Scatter"); plt.savefig("plot_scatter_11.png"); plt.close()

        plt.figure()
        sns.boxplot(data=self.data[["Sales","Profit"]]); plt.title("Box Plot")
        plt.savefig("plot_box_11.png"); plt.close()
...
def main():
    ...
    while True:
        ...
        if ch == "1":
            path = input("CSV path (or new path to auto-generate): ") or "sales_11.csv"
            tool.bring_csv(path); print("Loaded.")
        elif ch == "2":
            tool.view()
        elif ch == "3":
            tool.tidy_up(); print("Cleaned.")
        elif ch == "4":
            print("Sum of first five (demo):", tool.calc_set())
        elif ch == "5":
            print(tool.group_totals())
        elif ch == "6":
            print(tool.stat_kit())
        elif ch == "7":
            print(tool.pivot_do())
        elif ch == "8":
            tool.paint(); print("Saved plots.")
        elif ch == "9":
            print("Bye"); break
        else:
            print("Invalid")
